prompt PL/SQL Developer Export User Objects for user S4SPRES_CORE_DEV@DAPEX18
prompt Created by rmartens on Tuesday, March 31, 2020
set define off
spool install-db-objects.log

prompt
prompt Creating table DEMO_CODE
prompt ========================
prompt
...
compress;
alter table DEMO_CODE
  add constraint DEMO_CODE_PK primary key (ID);
alter table DEMO_CODE
  add constraint DEMO_CODE_STEP_UC unique (STEP);
...
compress;
alter table DEMO_CONSTRAINT_LOOKUP
...
prompt
prompt Creating table DEMO_TAGS
prompt ========================
prompt
...
prompt
prompt Creating table DEMO_TAGS_SUM
prompt ============================
prompt
...
compress;
alter table DEMO_TAGS_SUM
  add constraint DEMO_TAGS_SUM_PK primary key (TAG);

prompt
prompt Creating table DEMO_TAGS_TYPE_SUM
prompt =================================
prompt
...
compress;
alter table DEMO_TAGS_TYPE_SUM
  add constraint DEMO_TAGS_TYPE_SUM_PK primary key (TAG, CONTENT_TYPE);
...
prompt
prompt Creating table EBA_UT_CHART_PROJECTS
prompt ====================================
prompt
...
compress;
alter table EBA_UT_CHART_PROJECTS
  add constraint EBA_UT_CHARTS_PROJ_PK primary key (ID);

prompt
prompt Creating table EBA_UT_CHART_TASKS
prompt =================================
prompt
...
alter table EBA_UT_CHART_TASKS
  add constraint EBA_UT_CHART_TASKS_PK primary key (ID);
alter table EBA_UT_CHART_TASKS
  add constraint EBA_UT_CHART_TASKS_FK foreign key (PROJECT)
  references EBA_UT_CHART_PROJECTS (ID) on delete cascade;
...
prompt
prompt Creating table EMP_LETTERTEXTS
prompt ==============================
prompt
create table EMP_LETTERTEXTS
(
  id                NUMBER not null,
  empno             NUMBER,
  std_lettertext_id NUMBER,
  lettertext        CLOB,
  ordernr           NUMBER
)
compress;
alter table EMP_LETTERTEXTS
  add constraint ELTT_PK primary key (ID);

prompt
prompt Creating table KO_PRODUCTS
prompt ==========================
prompt
create table KO_PRODUCTS
(
  product_id          NUMBER not null,
  product_name        VARCHAR2(50),
  product_description VARCHAR2(2000),
...
compress;
alter table KO_PRODUCTS
  add constraint KO_PRODUCTS_PK primary key (PRODUCT_ID);

prompt
prompt Creating table STANDARD_LETTERTEXTS
prompt ===================================
prompt
create table STANDARD_LETTERTEXTS
(
  id         NUMBER not null,
  code       VARCHAR2(16),
  lettertext CLOB,
...
compress;
alter table STANDARD_LETTERTEXTS
  add constraint SLTT_PK primary key (ID);
...
prompt
prompt Creating sequence EMP_LETTERTEXTS_ID_SEQ
prompt ========================================
prompt
create sequence EMP_LETTERTEXTS_ID_SEQ
minvalue 1
maxvalue 9999999999999999999999999999
start with 902
increment by 1
...
prompt
prompt Creating sequence ISEQ$$_79643
prompt ==============================
prompt
create sequence ISEQ$$_79643
minvalue 1
maxvalue 9999999999999999999999999999
start with 1
increment by 1
...
prompt
prompt Creating sequence ISEQ$$_94329
prompt ==============================
prompt
create sequence ISEQ$$_94329
minvalue 1
maxvalue 9999999999999999999999999999
start with 1
increment by 1
...
prompt
prompt Creating view ALL_OBJECTS_VW
prompt ============================
prompt
create or replace force view all_objects_vw as
select alob.owner,
       alob.object_name,
       alob.subobject_name,
       alob.object_id,
       ...
       alob.sharing,
       alob.editionable,
       alob.oracle_maintained,
       alob.application,
       alob.default_collation,
       alob.duplicated,
       alob.sharded,
       alob.created_appid,
       alob.created_vsnid,
       alob.modified_appid,
       alob.modified_vsnid
  from all_objects alob;

prompt
prompt Creating view DEMO_CODE_NAMES_VW
prompt ================================
prompt
create or replace force view demo_code_names_vw as
select distinct
       demo_name
from demo_code
order by demo_name;
...
prompt
prompt Creating view DEMO_CODE_VW
prompt ==========================
prompt
create or replace force view demo_code_vw as
select doce.id                 as id
,      doce.demo_name          as demo_name
,      doce.step               as step
,      doce.step_name          as step_name
,      doce.html               as html
,      doce.css                as css
,      doce.js                 as js
,      lag(doce.id) over (partition by doce.demo_name order by doce.step)  as prev_step
,      lead(doce.id) over (partition by doce.demo_name order by doce.step) as next_step
from   demo_code doce;

prompt
prompt Creating package EMP_LETTERTEXT_PKG
prompt ===================================
prompt
create or replace package emp_lettertext_pkg is

  -- Author  : RMARTENS
  -- Created : 9/6/2019 1:16:50 AM
  -- Purpose : 

  -- Public type declarations
  procedure save_emp_lettertext;

end emp_lettertext_pkg;
/

prompt
prompt Creating package SAMPLE_DATA_PKG
prompt ================================
prompt
...
prompt
prompt Creating package SAMPLE_PKG
prompt ===========================
prompt
...
prompt
prompt Creating package body EMP_LETTERTEXT_PKG
prompt ========================================
prompt
create or replace package body emp_lettertext_pkg is

  -- remove starting-trailing spaces
  function trrim#(p_clob in clob, p_itterator in pls_integer) return clob is
    l_retval clob := p_clob;
  begin
    l_retval := trim(l_retval);
    l_retval := trim(chr(10) || chr(13) from l_retval);
  
    if p_itterator > 0
    then
      if p_clob != l_retval
      then
        l_retval := trrim#(l_retval, p_itterator - 1);
      end if;
    end if;
  ...
  procedure save_emp_lettertext is
  
    l_empno    varchar2(32767) := apex_application.g_x01;
    l_sltt_id  varchar2(32767) := apex_application.g_x02;
    l_ordernr  varchar2(32767) := apex_application.g_x03;
    l_text     varchar2(32767) := apex_application.g_x04;
    l_reccount pls_integer;
    --l_logscope varchar2(32767) := $$plsql_unit || '.save_emp_lettertext';
      
    cursor c_tekst(cp_sltt_id in standard_lettertexts.id%type) is
      select sltt.lettertext
      from   standard_lettertexts sltt
      where  sltt.id = cp_sltt_id;
  
  begin
  
    --logger.log(p_text => 'l_empno: ' || l_empno, p_scope => l_logscope);
    --logger.log(p_text => 'l_sltt_id: ' || l_sltt_id, p_scope => l_logscope);
    --logger.log(p_text => 'l_ordernr: ' || l_ordernr, p_scope => l_logscope);
  
    -- als het bftt-id gelijk is aan "-1", dan hebben we met een dummy entry te maken (uit het tweede deel van de union van de "brief"-region)
    if l_sltt_id = -1
    then
      -- als er nog geen briefteksten zijn, dan mag er iets gelogd worden
      select count(*)
      into   l_reccount
      from   emp_lettertexts eltt
      where  eltt.empno = l_empno;
    
      return;
    end if;
  
    -- als we de eerste brieftekst aan het behandelen zijn, dan mogen we de bestaande briefteksten eerst verwijderen
    if l_ordernr = 0
    then
      delete emp_lettertexts eltt where eltt.empno = l_empno;
    end if;
  
    if l_text is null
    then
      open c_tekst(l_sltt_id);
      fetch c_tekst
        into l_text;
      close c_tekst;
    else
      --l_text := crlftrim(l_text);
      l_text := regexp_replace(l_text, '<.*?>');
      l_text := replace(l_text, '&nbsp;', ' ');
      l_text := trrim#(l_text, 50);
    end if;
  
    -- vervolgens hoeven we natuurlijk alleen een insert te doen
    insert into emp_lettertexts
      (empno, std_lettertext_id, lettertext, ordernr)
    values
      (l_empno -- v_declaratie_id
      ,l_sltt_id -- v_hvva_brieftekst_id
      ,l_text -- v_hvva_brieftekst
      ,l_ordernr -- v_ordernr
       );
  
  end save_emp_lettertext;

end emp_lettertext_pkg;
/

prompt
prompt Creating package body SAMPLE_DATA_PKG
prompt =====================================
prompt
...
prompt
prompt Creating package body SAMPLE_PKG
prompt ================================
prompt
...
prompt
prompt Creating trigger BIU_EBA_UT_CHART_PROJ
prompt ======================================
prompt
...
prompt
prompt Creating trigger BIU_EBA_UT_CHART_TASKS
prompt =======================================
prompt
...
prompt
prompt Creating trigger DEMO_CUSTOMERS_BD
prompt ==================================
prompt
...
prompt
prompt Creating trigger DEMO_CUSTOMERS_BIU
prompt ===================================
prompt
...
prompt
prompt Creating trigger DEMO_ORDERS_BD
prompt ===============================
prompt
...
prompt
prompt Creating trigger DEMO_ORDERS_BIU
prompt ================================
prompt
...
prompt
prompt Creating trigger DEMO_ORDER_ITEMS_AIUD_TOTAL
prompt ============================================
prompt
...
prompt
prompt Creating trigger DEMO_ORDER_ITEMS_BI
prompt ====================================
prompt
...
prompt
prompt Creating trigger DEMO_ORDER_ITEMS_BIU_GET_PRICE
prompt ===============================================
prompt
...
prompt
prompt Creating trigger DEMO_PRODUCT_INFO_BD
prompt =====================================
prompt
...
prompt
prompt Creating trigger DEMO_PRODUCT_INFO_BIU
prompt ======================================
prompt
...
prompt
prompt Creating trigger DEMO_TAGS_BIU
prompt ==============================
prompt
...
prompt
prompt Creating trigger EM_LETTERTEXTS_BIUR_TRG
prompt ========================================
prompt
create or replace trigger em_lettertexts_biur_trg
  before insert or update on EMP_LETTERTEXTS
  for each row
declare
...
begin
  ...
  if :new.id is null
  then
    :new.id := emp_lettertexts_id_seq.nextval;
  end if;

end em_lettertexts_biur_trg;
/
